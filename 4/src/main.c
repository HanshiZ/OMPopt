...
int main(int argc, char *argv[])
{
    ...
    for(i = 0; i < nrowblks; i++)
        ...
        {
            seqblock blk1 = seqMatrixBlock[i*ncolblks + j];
            parblock blk2 = parMatrixBlock[i*ncolblks + j];
            if( blk1.nnz != blk2.nnz || blk1.roffset != blk2.roffset || blk1.coffset != blk2.coffset )
            {
                printf("%d %d %d %d %d\n", i, j, blk1.nnz, blk1.roffset, blk1.coffset);
                printf("%d %d %d %d %d\n", i, j, blk2.nnz, blk2.roffset, blk2.coffset);
            }
            for(int k = 0; k < blk1.nnz; k++)
                if(blk1.rloc[k] != blk2.rloc[k] || blk1.cloc[k] != blk2.cloc[k] || blk1.val[k] != blk2.val[k])
                    printf("%d %d %.4f %d %d %.4f\n",blk1.rloc[k], blk1.cloc[k], blk1.val[k], blk2.rloc[k], blk2.cloc[k], blk2.val[k]);
            //printf("\n");
